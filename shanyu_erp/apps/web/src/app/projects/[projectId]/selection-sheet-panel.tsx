...
export function SelectionSheetPanel({ quotationId }: { readonly quotationId: string }) {
  ...
  async function generate() {
    ...
    try {
      const job = await createSelectionSheetExport(quotationId);
  ...
  return <section className="grid gap-3">
    ...
    {preview ? <SelectionPdfPreview key={preview.url} url={preview.url} /> : null}
    <div className="flex flex-wrap justify-end gap-2 border-t pt-4">
      <Button variant={preview ? "outline" : "default"} disabled={busy} onClick={() => void generate()}>{busy ? "生成中…" : preview ? "重新生成" : "生成并预览选材单"}</Button>
      {preview ? <Button asChild><a href={preview.url} download={preview.record.fileName}>下载选材单 PDF</a></Button> : null}
    </div>
  </section>;
}
