...
const item = { id: "version-item", materialId: "stable", catalogVersionId: "bound", recordVersion: 2,
  categoryName: "淋浴房", categoryCode: "SHOWER", itemName: "淋浴房", attributes: {}, colors: ["亮银"],
...
describe("0920 customer selection sheet", () => {
  it("uses bound IDs and a strict output whitelist without price or source fields", async () => {
    ...
    expect(Object.keys(rows[0]!)).toEqual(["category", "name", "brand", "color", "spec", "description", "locations"]);
  ...
  it("renders a searchable portrait PDF with a confirmation page and no financial metadata", async () => {
    const rows = await buildSelectionSheetRows(quotation, repository);
    const file = await renderSelectionSheet({ templateVersion: "0920-v1", project: "隔离验收项目", customer: "测试客户", designer: "测试主案", version: 2, date: "2026/9/21", rows });
  ...
  it("paginates long descriptions and multiple categories without mutating the snapshot", async () => {
    const rows = await buildSelectionSheetRows(quotation, repository);
    const snapshot = { templateVersion: "0920-v1" as const, project: "分页验收", customer: "测试", designer: "测试", version: 1, date: "2026/9/21",
    ...
    const file = await renderSelectionSheet(snapshot);
    const pdf = await PDFDocument.load(file.payload);
  ...
  it("moves confirmation to a separate page when cover fields need more space", async () => {
    const rows = await buildSelectionSheetRows(quotation, repository);
    const snapshot = { templateVersion: "0920-v1" as const, project: "浙江省嘉兴市项目地址与楼栋房间说明".repeat(10), customer: "测试客户", designer: "测试主案", version: 1, date: "2026/9/21", rows };
    const before = JSON.stringify(snapshot);
    const file = await renderSelectionSheet(snapshot);
    const pdf = await PDFDocument.load(file.payload);
