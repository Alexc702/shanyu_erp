...
export async function reconcileSafeDrafts(database: DatabaseExecutor, options: {
  ...
  if (options.apply) await database.query("LOCK TABLE main_material_catalog_versions IN SHARE ROW EXCLUSIVE MODE");
  ...
  for (const q of quotes) {
    const entry: SafeUpdateEntry = { projectId: q.project_id, projectAddress: String(q.project_address), quotationId: q.id, version: q.version_number, status: q.status,
      revision: q.revision, fromCatalogId: q.main_material_catalog_version_id, outcome: "PROTECTED", differences: [] };
    entries.push(entry);
    ...
    if (q.main_material_catalog_version_id === target.id) { entry.outcome = "CURRENT"; continue; }
    ...
    if (entry.differences.length) { entry.outcome = "BLOCKED"; continue; }
    entry.outcome = "SAFE";
  ...
  return { policy: "zero-impact-v1", targetCatalogId: target.id, targetVersion: target.version_number, summary, entries };
}
