...
### 035 单商品历史主图修正（必须单独授权）

仅本次明确审核 `035_shower_34a_image` 时，发布清单增加
`"imageCorrection": "035_shower_34a_image"`。省略则维持普通逐行保护，不允许图片变化。
此白名单固定为 `MAT-SHOWER-DC6F85FFDF14`，旧图 `547f7108b0fc470f638bdb35cb6db252621ea2a0417b7b4d57a18a5768776029`
替换为 `b81f83d09bc071504448c4d6f5a9dae0c2b0770ced1aad8292a6ef374a579766`。
不能配置其他商品或任意忽略字段。

候选镜像的基线工具在迁移前计算独立的035预期行摘要：新主图置首，移除旧/新图重复项，
其他图片与色卡保持原序。迁移后逐行比对完整预期摘要，不删除 `asset_ids` 参与校验。
同步核对全目录商品/版本/导入批次、全部资产和图片关联、新图登记和文件SHA、两个历史
保护触发器启用状态及定义。后续主材安全更新仍单独验证只允许的引用变更；失败保留维护。
035改变业务内容哈希，必须从隔离候选库取得新哈希，不能沿用032哈希。

本地专项回归入口为 `apps/api/scripts/test-deployment-image-035.mjs`，显式设置
`SHANYU_IMAGE035_TEST=1`，仅允许 `127.0.0.1:55435/shanyu_image035_test` 的一次性合成库
（重复演练可使用 `_` 加小写字母/数字后缀的新库）。
先用已缓存032版本API镜像建立schema，不使用日常数据库或云端数据；该回归准备草稿、
已报价/退回/批准快照及共享旧图的非目标商品，然后执行033–035并注入非法差异。
部署故障停止测试仍由 `scripts/deployment/tests/guarded-upgrade.test.py` 覆盖。
正式环境必须独立授权；独立选材单真实图片输出和完整0920候选库发布不在此修正范围。
